...
/*
 * La home tiene dos formas.
 *
 * Sin sesión es una portada: el hero cuenta qué es LetterRock, y abajo van los
 * artistas, lo que se escucha y las colecciones, para mostrar que hay vida.
 *
 * Con sesión eso ya lo sabés, y "Crear mi cuenta" no te sirve: arranca directo
 * en el feed, con las colecciones y los artistas populares achicados al
 * costado.
 */
export default function Home() {
  const user = useAuthStore(s => s.user)
  const session = useAuthStore(s => s.session)
  // Con la sesión detectada alcanza: el perfil puede tardar un poco más, y
  // esperar a `user` mostraría el hero un instante antes de saltar al feed.
  const signedIn = !!user || session === 'present'
  ...
  // Todavía no se sabe si hay sesión (es local, tarda un instante). Mejor un
  // hueco que mostrar la portada y reemplazarla por el feed enfrente tuyo.
  if (session === 'unknown') return <div className="min-h-[60vh]" />

  const FeedTitle = signedIn ? 'h1' : 'h2'

  const feed = (
    <section className="flex-1 min-w-[320px]">
      <div className="flex items-baseline gap-4 flex-wrap mb-5">
        <FeedTitle className="font-display text-3xl">Lo que se está escuchando</FeedTitle>
  ...
  const collectionsBlock = sideCollections.length > 0 && (
    <div>
  ...
  /* — Con sesión: el feed primero — */
  if (signedIn) {
    // Los errores y el vacío de artistas no se anuncian en el costado: ahí
    // son un extra, y un cartel de error al lado del feed distrae de lo que
    // sí cargó. Si no hay nada, el bloque no aparece.
    const sideArtists = popularArtists.slice(0, 6)

    return (
      <div className="animate-fade-up flex flex-wrap gap-11 pt-8 md:pt-10">
        {feed}

        <aside className="flex-1 min-w-[260px] max-w-[360px] space-y-10">
          {collectionsBlock}

          {(loading || sideArtists.length > 0) && (
            <div>
              <div className="flex items-center gap-3 flex-wrap mb-4">
                <h3 className="font-display text-xl">Artistas populares</h3>
                <ArrowLink to="/search" className="ml-auto">Ver todos</ArrowLink>
              </div>
              {loading ? (
                <SkeletonGrid count={6} min={90} />
              ) : (
                <div className="grid grid-cols-3 gap-x-4 gap-y-5">
                  {sideArtists.map(a => (
                    <ArtistCard key={a.id} artist={a} variant="circle" showOrigin={false} compact />
                  ))}
                </div>
              )}
            </div>
    ...
    <div className="animate-fade-up">
      ...
      <div className="flex flex-wrap gap-11">
        ...
        {sideCollections.length > 0 && (
          <aside className="flex-1 min-w-[260px] max-w-[360px]">{collectionsBlock}</aside>
        )}
      </div>
    </div>
  )
